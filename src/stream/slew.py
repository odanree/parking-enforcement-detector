...
polygons in ``config/rodent.yaml`` under ``slew_presets``. First polygon that
contains the bbox center wins.
...
debounce-coalescer idea as the vector-store dedup, but for PTZ commands — so
the secondary doesn't thrash on every frame of one rodent.

Env vars:
  SECONDARY_CAMERA_ID   Which PTZ camera_id to command (default: 1).
  SLEW_LOCKOUT_SECONDS  Per-event debounce window in seconds (default: 10).
  RODENT_SLEW_ENABLED   Master kill-switch; enforced by the strategy (see
                        strategy.py). This module always dispatches when
                        called; the strategy is what gates it.
"""
...
class SlewDispatcher:
    ...
    Constructed with the parsed rodent config; call ``slew_to_bbox`` when a
    positive fires. Thread-safe (pipeline harvests VLM jobs on the main loop,
    but a test or admin path could call from another thread).
    """
    ...
    def slew_to_bbox(
        ...
        """Look up the zone for the bbox center and issue a GotoPreset.

        Returns True if the PTZ command was sent, False if it was suppressed
        ...
        with self._lock:
            ...
            if now - last < self.lockout_seconds:
                logger.debug(
                    "slew: lockout active for %s (%.1fs < %.1fs) — no-op",
        ...
        if ok:
            logger.info(
                "slew: cam=%d preset=%d zone=%s bbox_center=(%d,%d) event=%s",
                self.camera_id, zone.preset, zone.name, cx, cy, event_key,
            )
        else:
            logger.warning(
                "slew: PTZ preset command FAILED cam=%d preset=%d zone=%s",
                self.camera_id, zone.preset, zone.name,
            )
            # Roll back the lockout so a transient failure doesn't lose the next event.
            with self._lock:
...
def _point_in_polygon(x: int, y: int, poly: list[tuple[int, int]]) -> bool:
    """Ray-casting point-in-polygon. poly is a list of (x, y) pairs."""
    if len(poly) < 3:
        return False
    inside = False
    n = len(poly)
...
def get_dispatcher(config_path: str = "config/rodent.yaml") -> SlewDispatcher:
    """Return a process-wide dispatcher, lazily built from rodent.yaml."""
    global _dispatcher
    with _dispatcher_lock:
        if _dispatcher is None:
            ...
            logger.info(
                "slew: dispatcher initialized cam=%d zones=%d lockout=%.1fs",
