...
Tests the zone→preset lookup, per-event lockout debounce, and failure rollback.
No real PTZ commands issued — ``ptz_preset`` is patched.
"""
...
class TestSlewLockout:
    ...
    def test_ptz_failure_rolls_back_lockout(self):
        # If the PTZ command fails, the lockout must NOT stick — otherwise a
        # single transient failure would drop the entire event window.
        d = self._dispatcher(lockout=10.0)
        with patch("src.stream.slew.ptz_preset", return_value=False) as mock_ptz:
            ok = d.slew_to_bbox(bbox=(100, 100, 200, 200), frame_width=1280, frame_height=720,
                                event_key=("rodent", 42))
            assert ok is False
        # Next attempt with a working PTZ should immediately fire again.
